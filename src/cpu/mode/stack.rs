...
fn push<'a>(cpu: &'a Cpu, value: u8) -> impl Generator<Yield=CpuCycle, Return=()> + 'a {
    move || {
...
pub(in crate::cpu) fn php<'a>(cpu: &'a Cpu) -> impl Generator<Yield=CpuCycle, Return=()> + 'a {
    // PHP sets the B flag
    let mut flags = cpu.flags.get();
    flags.set(Flags::B, true);
    push(cpu, flags.bits())
}
pub(in crate::cpu) fn pha<'a>(cpu: &'a Cpu) -> impl Generator<Yield=CpuCycle, Return=()> + 'a {
...
pub(in crate::cpu) fn plp<'a>(cpu: &'a Cpu) -> impl Generator<Yield=CpuCycle, Return=()> + 'a {
    move || {
...
pub(in crate::cpu) fn pla<'a>(cpu: &'a Cpu) -> impl Generator<Yield=CpuCycle, Return=()> + 'a {
    move || {
        let value = yield_complete!(pull(cpu));
        cpu.acc.set(value);
